...
export async function createNewDocument(repoName: string) {
  ...
  //   console.log(`http://192.168.0.103:5000/reponame?query=${repoName}`);

  const res = await axios.post(
    `http://192.168.0.103:5000/api/reponame?query=${repoName}`
  );

  console.log(res.data);

  const docRef = await addDoc(
    collection(db, `users/${sessionClaims?.email!}/repos`),
    res.data
  );
